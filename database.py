...
def init_db() -> None:
    ...
    c.executemany(
        "INSERT OR IGNORE INTO categories (name_uz, name_ru) VALUES (?, ?)",
        default_cats
    )
    conn.commit()
    conn.close()
...
# ── Users ─────────────────────────────────────────────────
def save_user(user_id: int, username: str | None, full_name: str, lang: str = "uz") -> None:
    conn = get_conn()
    conn.execute(
...
# ── Movies ────────────────────────────────────────────────
def add_movie(name_uz: str, name_ru: str, category_id: int,
              file_id: str, year: int | None,
              desc_uz: str, desc_ru: str, admin_id: int) -> int:
    conn = get_conn()
    cur = conn.execute(
        """INSERT INTO movies
           (name_uz,name_ru,category_id,file_id,year,description_uz,description_ru,added_by)
           VALUES (?,?,?,?,?,?,?,?)""",
        (name_uz, name_ru, category_id, file_id, year, desc_uz, desc_ru, admin_id)
    )
    conn.commit()
    mid = cur.lastrowid
    conn.close()
...
def get_movies_by_category(cat_id: int, limit: int = 50, offset: int = 0) -> list:
    conn = get_conn()
    rows = conn.execute(
        """SELECT m.*, c.name_uz as cat_uz, c.name_ru as cat_ru
           FROM movies m LEFT JOIN categories c ON m.category_id=c.id
