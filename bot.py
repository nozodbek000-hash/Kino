...
async def start(message: Message, state: FSMContext):
    ...
    if not await check_subscription(uid):
        await message.answer(
            "👋 Xush kelibsiz!\n\nKinolardan foydalanish uchun kanalga obuna bo'ling:",
            reply_markup=sub_kb()
        )
        return
    await message.answer(
...
async def check_sub(call: CallbackQuery):
    ...
    await call.message.edit_text(
        f"✅ Obuna tasdiqlandi!\n\n👋 Xush kelibsiz, <b>{call.from_user.first_name}</b>!",
        reply_markup=main_kb(is_admin(call.from_user.id)),
        parse_mode="HTML"
    )
...
async def search_result(message: Message, state: FSMContext):
    await state.clear()
    results = db.search_movie_by_name(message.text.strip())
    if not results:
        await message.answer(
            f"❌ '{message.text}' bo'yicha kino topilmadi!",
...
async def by_code_start(call: CallbackQuery, state: FSMContext):
    await state.clear()
    await state.set_state(CodeMovie.code)
    await call.message.edit_text("🔑 Kino kodini kiriting (masalan: K001):", reply_markup=back_kb())


@dp.message(CodeMovie.code)
async def by_code_result(message: Message, state: FSMContext):
    await state.clear()
...
async def add_start(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        "➕ Kino qo'shish\n\n1️⃣ Kino kodini kiriting (masalan: K001):",
...
async def handle_any(message: Message):
    ...
    if movie:
        ...
        await message.answer("❓ /start bosing yoki to'g'ri kod kiriting.")
