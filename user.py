...
from config import PER_PAGE, CHANNEL_ID, STORAGE_CHANNEL, ADMIN_IDS
from database import (
    save_user, get_user_lang, set_user_lang,
    search_movies, get_movies_by_category, count_by_category,
    get_movie, get_new_movies, get_category,
)
...
async def is_subscribed(user_id):
    if is_admin(user_id):
        return True
    if CHANNEL_ID is None:          # Yangi qo'shilgan qator
        return True                 # Majburiy obunani o'chiramiz
    bot = Bot.get_current()
    try:
...
async def check_sub_cb(cb: types.CallbackQuery):
    if await is_subscribed(cb.from_user.id):
        ...
        lang = get_user_lang(cb.from_user.id)
        await cb.answer(
            "❌ Hali obuna bo'lmadingiz!" if lang == "uz" else "❌ Вы ещё не подpisались!",
            show_alert=True
        )
...
async def search_process(message: types.Message, state: FSMContext):
    ...
    kb = movies_list_kb(movies[:PER_PAGE], 0, len(movies), PER_PAGE, "srch", lang)
...
async def movie_cb(cb: types.CallbackQuery):
    ...
    if not movie:
        ...
        return
    await cb.message.answer(movie_text(movie, lang), parse_mode="HTML", reply_markup=movie_detail_kb(movie_id, lang))
    await cb.answer()
...
async def send_movie_by_id(message: types.Message, movie_id: int, lang: str):
    ...
    if not movie:
        ...
        return
    await message.answer(movie_text(movie, lang), parse_mode="HTML", reply_markup=movie_detail_kb(movie_id, lang))
